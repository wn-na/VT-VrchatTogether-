...
// common component
import { Container, Header, Content, Footer, Button, Left, Right, Body, Item, Label, Input, H2, H1, Badge, Text, SwipeRow, Picker, Textarea, Fab, List, ListItem, Switch, Drawer } from 'native-base';
import { Image, StyleSheet, SectionList, FlatList, TouchableOpacity, ScrollView, View, TextInput, Dimensions, Alert, Modal, AsyncStorage, ToastAndroid } from 'react-native';
...
export const FavoriteWorld = new Map();
export const FavoriteWorldTag = new Map();
        ...
        .then((responseJson) => {
            if (!responseJson.error) {
                ...
                if (FavoriteWorldTag.size < 4) {
                    for (let i = FavoriteWorldTag.size; i < 4; i++) FavoriteWorldTag.set(`worlds${i + 1}`, `worlds${i + 1}`);
                }
            }
        });
}
...
drawWorldTagList = (state, item) => {
    return [...FavoriteWorldTag.keys()].map((element, idx) => (
        <Button key={FavoriteWorldTag.get(element)} onPress={() => this.addFavoriteWorld(state, item, element)} style={[styles.groupButton, { width: '90%' }]}>
...
export function drawModal(state) {
    return (
        <Modal animationType="slide" transparent={true} visible={!!state.display || false} onBackButtonPress={() => state.toggleModal()} onBackdropPress={() => state.toggleModal()}>
            <View
                style={{
                    flex: 1,
                    alignItems: 'center',
                ...
                <View
                    ...
                    <Button style={[styles.requestButton, { width: '80%', height: 40, margin: 10, justifyContent: 'center' }]} onPress={() => state.toggleModal()}>
...
DrawMap = (state, item) => (
    <View style={{ borderWidth: 1 }}>
        ...
        <View style={{ marginLeft: '3%' }}>
            <Text>
                {translate('map_name')} : {item.name}
            </Text>
            <Text>
                {translate('map_info')} : {item.releaseStatus}
            </Text>
            {item.publicOccupants !== undefined ? (
                <Text>
                    {translate('map_user')} : {item.publicOccupants}
                </Text>
            ) : null}
            <Text>
                {translate('map_all_user')} : {item.occupants}
            </Text>
            <Text>
                {translate('map_last_update')} : {Moment(item.updated_at).format('LLLL')}
            </Text>
        </View>
    </View>
);
