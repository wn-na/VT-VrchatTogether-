...
async function getFriendOn() {
    ...
    return new Promise((resolve, reject) => resolve(responseOn));
}
...
async function getFriendActive() {
    ...
    return new Promise((resolve, reject) => resolve(responseActive));
}
...
async function getFriendOff() {
    ...
    return new Promise((resolve, reject) => resolve(responseOff));
}

export function friendRequest(state, notiId, type) {
    if (type == true) {
        Alert.alert(translate('information'), translate('msg_friendrequest'), [
            {
                text: translate('ok'),
                onPress: () => {
        ...
        Alert.alert(translate('information'), translate('msg_friendrequest_deny'), [
            {
                text: translate('ok'),
                onPress: () => {
...
export async function changeStatus(state, statusText) {
    if (statusText.length > 200) {
        ...
        await fetch(
            `https://api.vrchat.cloud/api/1/users/${userInfo.id}`,
            VRChatAPIPutBody({
