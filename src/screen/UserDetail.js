...
import { Image, ScrollView, View, Alert, ToastAndroid, ActivityIndicator, TouchableOpacity } from 'react-native';
...
export default class UserDetail extends Component {
    ...
    friendRequest(id, type) {
        if (type == true) {
            Alert.alert(translate('information'), translate('msg_delete_friend'), [
                {
                    text: translate('ok'),
                    onPress: () => {
            ...
            Alert.alert(translate('information'), translate('msg_send_friendrequest'), [
                {
                    text: translate('ok'),
                    onPress: () => {
    ...
    block() {
        if (this.state.isBlocked == false) {
            Alert.alert(translate('information'), translate('msg_block_user'), [
                {
                    text: translate('ok'),
                    onPress: () => {
                        fetch(
            ...
            Alert.alert(translate('information'), translate('msg_release_block_user'), [
                {
                    text: translate('ok'),
                    onPress: () => {
                        fetch(
        ...
        return (
            <View style={[styles.mainBackground, { flex: 1 }]}>
                <View style={[styles.logo, { justifyContent: 'center' }]}>
                    <Icon onPress={() => Actions.pop()} name="chevron-left" size={25} style={{ color: 'white', position: 'absolute', left: 15, top: 10 }} />
                        ...
                        <View style={{ flex:1 }}>
                            <View style={{ flexDirection:"row",justifyContent:"space-between",marginLeft:"5%",marginRight:"5%",marginTop:"1%" }}>
                                ...
                                <View style={{ flexDirection:"row",marginTop:10 }}>
                                    <TouchableOpacity
                                        onPress={() => {
                                            Actions.currentScene == 'userDetail' && Actions.makeDetail({ userId: this.props.userId });
                                        }}>
                                        <Image style={{ width: 30, height: 30, marginRight: 30 }} source={require('../css/imgs/make_detail_icon.png')} />
                                    </TouchableOpacity>
                                    {this.state.isBlocked == true ? (
                                        <Icon onPress={this.block.bind(this)} name={'block'} size={30} style={{ color: '#ef5261' }} />
                                    ) : (
                                        <Icon onPress={this.block.bind(this)} name={'block'} size={30} style={{ color: '#888c8b' }} />
                                    )}
                                </View>
                            </View>
                                ...
                                <View style={{ width:"100%",marginLeft:"3%" }}>
                                    <NetmarbleL style={styles.friendInfoText}>
                                        ...
                                        {this.state.getUserInfo.statusDescription != '' &&
                                            this.state.getUserInfo.statusDescription != null &&
                                ...
                                <View style={styles.worldInfoDetail}>
                                    <View style={{ alignItems:"flex-end" }}>
                                        {this.state.isFavorite == true ? (
                                            <TouchableOpacity onPress={this.favorite.bind(this, -1, this.state.favoriteId)}>
                                                <Image source={require('../css/imgs/favorite_star.png')} style={{ width: 30, height: 30 }} />
                                            </TouchableOpacity>
                                        ) : (
                                            <TouchableOpacity onPress={() => this.setState({ modalVisible: true })}>
                                                <Image source={require('../css/imgs/unfavorite_star.png')} style={{ width: 30, height: 30 }} />
                                            </TouchableOpacity>
                                        )}
                                    </View>
                                    ...
                                    <View style={{ marginTop:"2%",marginBottom:"4%" }}>
                                        <Image style={{ width: '100%', height: 200, borderRadius: 20 }} source={VRChatImage(this.state.getUserWInfo.thumbnailImageUrl)} />
                                    </View>
                                    <NetmarbleL style={[styles.requestButtonText, { lineHeight:25 }]}>
                                        {translate('creator')} : {this.state.getUserWInfo?.authorName}
                                        {'\n'}
                                        {translate('online_world_user')} :{' '}
                                        {this.state.indiInfo?.length != 0
                                            ? this.state.indiInfo[0][1] + '/' + this.state.getUserWInfo?.capacity
                                ...
                                <Modal
                                    ...
                                    {this.state.modalVisible == true ? (
                                        <View style={{ backgroundColor: '#fff', borderRadius: 10 }}>
                                            <Button style={styles.groupButton} onPress={this.favorite.bind(this, 0, this.state.getUserWInfo.id)}>
                                                <NetmarbleL>Group 1</NetmarbleL>
                                            </Button>
                                            <Button style={styles.groupButton} onPress={this.favorite.bind(this, 1, this.state.getUserWInfo.id)}>
                                                <NetmarbleL>Group 2</NetmarbleL>
                                            </Button>
                                            <Button style={styles.groupButton} onPress={this.favorite.bind(this, 2, this.state.getUserWInfo.id)}>
                                                <NetmarbleL>Group 3</NetmarbleL>
                                            </Button>
                                            <Button style={styles.groupButton} onPress={this.favorite.bind(this, 3, this.state.getUserWInfo.id)}>
